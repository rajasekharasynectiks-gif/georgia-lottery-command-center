    ...
    <div className="min-h-screen flex bg-background text-foreground">
      ...
      <aside className="hidden lg:flex w-64 flex-col border-r border-sidebar-border bg-sidebar sticky top-0 h-screen">
        <div className="h-16 flex items-center gap-3 px-5 border-b border-sidebar-border">
          <div className="size-9 rounded-md bg-gradient-to-br from-primary to-amber-700 grid place-items-center text-primary-foreground font-display font-bold shadow-[var(--shadow-glow)]">GL</div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
          {nav.map((g) => (
            <div key={g.group}>
              <div className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground/80">{g.group}</div>
                  ...
                  return (
                    <li key={it.to}>
                      <Link to={it.to} className={
                        ...
                        {active && <span className="size-1.5 rounded-full bg-primary shadow-[0_0_10px_var(--color-primary)]" />}
                      </Link>
                    </li>
                  );
                })}
        ...
        <div className="px-4 py-3 border-t border-sidebar-border text-[11px] text-muted-foreground">
          ...
          <div className="mt-1 font-mono text-[10px] opacity-60">env: prod-us-east • v5.0.1</div>
        </div>
      </aside>
